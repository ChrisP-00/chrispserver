...
CREATE TABLE IF NOT EXISTS gameserverdb.`user_character` (
    user_index	                INT	         NOT NULL PRIMARY KEY   COMMENT '사용자 고유 식별자',
    character_index	            INT	         NOT NULL UNIQUE KEY    COMMENT '캐릭터 고유 식별자',
    level	                    INT	         NOT NULL DEFAULT TRUE  COMMENT '캐릭터 레벨',
    exp	                        INT	         NOT NULL DEFAULT FALSE COMMENT '캐릭터 경험치',
    dirt_level	                INT	         NOT NULL DEFAULT FALSE COMMENT '더러움 레벨',
    quantity	                INT	         NOT NULL DEFAULT TRUE  COMMENT '캐릭터 보유 수량',
...
CREATE TABLE IF NOT EXISTS gameserverdb.`user_goods` (
    user_index	                INT	        NOT NULL PRIMARY KEY    COMMENT '사용자 고유 식별자',
    goods_index	                TINYINT     NOT NULL UNIQUE KEY     COMMENT '재화 고유 식별자',
    quantity	                INT		    NOT NULL                COMMENT	'보유 개수'  
) COMMENT='유저 재화 테이블';
```
...
CREATE TABLE IF NOT EXISTS gameserverdb.`user_inventory` (
    user_index                  INT         NOT NULL PRIMARY KEY               COMMENT '사용자 고유 식별자',
    item_index                  INT         NOT NULL UNIQUE KEY                COMMENT '아이템 고유 식별자',
...
CREATE TABLE IF NOT EXISTS gameserverdb.`user_daily_mission` (
    ...
    amount                TINYINT    NOT NULL DEFAULT 0 COMMENT '수행 횟수',
...
CREATE TABLE IF NOT EXISTS gameserverdb.`user_character_mission` (
    ...
    amount                    INT        NOT NULL COMMENT '수행 횟수',
    is_received              BOOL       NOT NULL DEFAULT FALSE COMMENT '수령 여부',
